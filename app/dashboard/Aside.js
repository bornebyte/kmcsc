    ...
    return (
        <aside className='w-full bg-gray-900'>
            <div className="flex justify-end items-center text-white pr-12 text-4xl pb-6">
                {toggleSideBar ? <AiOutlineClose className="hover:cursor-pointer" onClick={() => setToggleSideBar(!toggleSideBar)} /> : <GiHamburgerMenu className="hover:cursor-pointer" onClick={() => setToggleSideBar(!toggleSideBar)} />}
            </div>
            {
                toggleSideBar && <div className="flex flex-col gap-6 py-8 px-6 min-h-screen">
                    ...
                    <Link onClick={() => setToggleSideBar(false)} href={"/dashboard/passwordresetrequest"} className='cursor-pointer w-full lg:w-[40vw] mx-auto h-12 bg-gray-800 text-white font-bold rounded-lg px-2 py-1 text-center flex justify-center items-center' ><div className="flex items-center justify-center gap-6"><div><IoMdNotifications className="text-2xl" /></div><div>Password reset request</div></div></Link>
